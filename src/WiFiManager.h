...
void startAccessPoint()
{
  ...
  server.on("/validarCredenciais", HTTP_POST, [](AsyncWebServerRequest *request)
            {
    if (request->hasParam("username", true) && request->hasParam("senha", true)) {
      ...
      String usernameSalvo = preferences.getString("username", "");
      ...
      if (senha == senhaSalva && username == usernameSalvo) {
